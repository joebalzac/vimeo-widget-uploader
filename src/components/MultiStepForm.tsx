...
declare global {
  interface Window {
    DefaultSDK?: {
      submit: (submission: DefaultSubmission, callbacks?: DefaultCallbacks) => Promise<void>;
      helloWorld: () => void;
    };
  }
}
...
const UNITS_MANAGED_OPTIONS = ["<450", "450-999", "1,000-4,999", "5,000-15,999", "16,000+"];
const PMS_OPTIONS = ["Appfolio", "Buildium", "Entrata", "MRI", "Propertyware", "RealPage", "ResMan", "Yardi", "More than one", "Other"];

const INITIAL_FORM: FormData = {
  email: "", firstname: "", lastname: "", phone: "",
  company: "", units_managed: "", pms_compatability: "",
...
const BLOCKED_DOMAINS = new Set([
  "gmail.com","googlemail.com","yahoo.com","yahoo.co.uk","yahoo.co.in","yahoo.fr","yahoo.es","yahoo.de",
  "hotmail.com","hotmail.co.uk","hotmail.fr","hotmail.es","hotmail.de","outlook.com","outlook.co.uk","outlook.fr",
  "live.com","live.co.uk","live.fr","msn.com","icloud.com","me.com","mac.com","aol.com",
  "protonmail.com","proton.me","mail.com","email.com","zoho.com","yandex.com","yandex.ru",
  "gmx.com","gmx.de","gmx.net","tutanota.com","tutamail.com","fastmail.com","fastmail.fm","hey.com","duck.com",
]);
...
async function createContact(email: string): Promise<void> {
  ...
  } catch (err) { console.warn("[createContact] failed silently", err); }
}

async function updateContact(email: string, properties: Record<string, string>): Promise<void> {
  try {
    await fetch(`${API_BASE}/api/update-contact`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, properties }),
    });
  } catch (err) { console.warn("[updateContact] failed silently", err); }
}
...
const slideVariants = {
  ...
  exit:   (dir: number) => ({ x: dir > 0 ? -60 : 60, opacity: 0, transition: { duration: 0.25, ease: [0.25, 0.46, 0.45, 0.94] } }),
};
...
function ContentPanel({
  contentHeadline, contentBody, contentImageUrl, contentImageAlt,
  contentBackgroundColor, contentOnlyLogoUrl, multipleLogos,
  contentOneLogoUrl, contentTwoLogoUrl, contentThreeLogoUrl, contentLogoAlt,
  mainQuote, testimonialQuote, testimonialName, testimonialTitle,
  testimonialCompany, testimonialAvatarUrl,
}: ContentPanelProps) {
  const bgStyle = contentImageUrl
    ? { backgroundImage: `url(${contentImageUrl})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat" }
      ...
      <div className="msf__content-inner">
        ...
        {(contentHeadline || contentBody) && (
          <div className="msf__content-headline-body">
            {contentHeadline && <h2 className="msf__content-headline">{contentHeadline}</h2>}
        ...
        {contentOnlyLogoUrl && !multipleLogos && (
          <img src={contentOnlyLogoUrl} alt={contentLogoAlt} className="msf__content-logo" />
        )}

        {/* Multiple logos */}
        {multipleLogos && (
          <div className="msf__content-logos">
            {contentOneLogoUrl   && <div className="msf__content-logo-wrap"><img src={contentOneLogoUrl}   alt={contentLogoAlt} className="msf__content-logo-img" /></div>}
            {contentTwoLogoUrl   && <div className="msf__content-logo-wrap"><img src={contentTwoLogoUrl}   alt={contentLogoAlt} className="msf__content-logo-img" /></div>}
            {contentThreeLogoUrl && <div className="msf__content-logo-wrap"><img src={contentThreeLogoUrl} alt={contentLogoAlt} className="msf__content-logo-img" /></div>}
          </div>
        )}
          ...
          <div className="msf__main-quote">
            ...
            {(testimonialName || testimonialAvatarUrl) && (
              <div className="msf__testimonial-author">
                {testimonialAvatarUrl && <img src={testimonialAvatarUrl} alt={testimonialName} className="msf__testimonial-avatar" />}
                <div className="msf__testimonial-meta">
                  {testimonialName    && <span className="msf__testimonial-name">{testimonialName}</span>}
                  {testimonialTitle   && <span className="msf__testimonial-title">{testimonialTitle}</span>}
                  {testimonialCompany && <span className="msf__testimonial-company">{testimonialCompany}</span>}
                </div>
              </div>
            )}
          </div>
...
export default function MultiStepForm({
  ...
  const [form, setForm]             = useState<FormData>({ ...INITIAL_FORM, email: initialEmail || "" });
  const [errors, setErrors]         = useState<Partial<Record<keyof FormData, string>>>({});
  const [submitted, setSubmitted]   = useState<boolean>(false);
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [apiError, setApiError]     = useState<string>("");

  useEffect(() => { loadDefaultSDK().catch(err => console.warn("[Default SDK]", err)); }, []);
  ...
  const validateStep = useCallback((): boolean => {
    ...
    if (step === 1) {
      ...
      else if (!validateEmail(form.email)) errs.email = "Please use your work email address.";
    }
    if (step === 2) {
  ...
  const next = useCallback(() => {
    ...
    setDir(1);
    setStep(s => { const n = Math.min(s + 1, TOTAL_STEPS); onStepChange?.(n); return n; });
  }, [validateStep, onStepChange]);

  const prev = useCallback(() => {
    setDir(-1);
    setStep(s => { const p = Math.max(s - 1, 1); onStepChange?.(p); return p; });
  }, [onStepChange]);
  ...
  const submit = useCallback(async () => {
    ...
    const fields: HubSpotField[] = [
      ...
      { name: "in_which_areas_of_your_operations_are_you_looking_to_implement_ai_", value: form.in_which_areas_of_your_operations_are_you_looking_to_implement_ai_ },
      ...(promoOffering ? [{ name: "promo_offering", value: promoOffering }] : []),
    ];

    (["utm_source", "utm_medium", "utm_campaign"] as const).forEach(p => {
      const v = getParam(p); if (v) fields.push({ name: p, value: v });
    });
    ...
    try {
      ...
      window.DefaultSDK!.submit({
        form_id: DEFAULT_FORM_ID, team_id: DEFAULT_TEAM_ID,
        responses: {
          email: form.email, firstname: form.firstname, lastname: form.lastname,
          phone: form.phone, company: form.company, units_managed: form.units_managed,
        ...
        onMeetingBooked:      d  => console.log("[Default] meeting booked", d.payload),
      });
  ...
  const hasContentPanel = !!(
    contentImageUrl || contentBackgroundColor || contentHeadline ||
    contentBody || contentOnlyLogoUrl || multipleLogos || mainQuote || testimonialQuote
  );
    ...
    <>
              ...
              <div className={`emailCapture${errors.email ? " emailCapture--error" : ""}`}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (form.email && validateEmail(form.email)) void createContact(form.email);
                    next();
                  }}
                >
                  {emailCTAText}
            ...
            {enableLowbrow && (
              <div className="hsf__lowbrow-container">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                  ...
                  <defs><clipPath id="g2_clip"><rect width="16" height="16" fill="white"/></clipPath></defs>
                </svg>
                <span className="hsf__separator"></span>
                <div className="lowbrow-check-wrapper">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <g clipPath="url(#soc2_clip)">
                      <path d="M7.99998 15.4286C12.1027 15.4286 15.4286 12.1027 15.4286 7.99998C15.4286 3.8973 12.1027 0.571411 7.99998 0.571411C3.8973 0.571411 0.571411 3.8973 0.571411 7.99998C0.571411 12.1027 3.8973 15.4286 7.99998 15.4286Z" stroke="#515152" strokeLinecap="round" strokeLinejoin="round"/>
                      <path d="M4.99994 8.16663L6.99994 10.1666L10.9999 6.16663" stroke="#515152" strokeLinecap="round" strokeLinejoin="round"/>
                    </g>
                    <defs><clipPath id="soc2_clip"><rect width="16" height="16" fill="white"/></clipPath></defs>
                  </svg>
                  <p className="hsf__lowbrow-text">SOC2 Type II</p>
                </div>
              </div>
            )}
          </div>
      ...
      <AnimatePresence>
        {step > 1 && (
          ...
          >
            <div className={`hsf__split${hasContentPanel ? " hsf__split--with-panel" : ""}`}>
              ...
              <div className="hsf__form-col" onKeyDown={onKeyDown}>
                <div className="hsf__overlay-inner">
                  ...
                  <div className="hsf__progress-track" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}>
                    <motion.div className="hsf__progress-fill" initial={false} animate={{ width: `${progress}%` }} transition={{ duration: 0.4, ease: "easeInOut" }} />
                  </div>
                  ...
                  <div className="hsf__nav-back-container">
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (step === 2 && onBack) onBack(); else prev();
                      }}
                    >
                          ...
                          <div className="hsf__fields">
                            <div className="hsf__row">
                              <div className="hsf__col">
                                ...
                                <input id="hsf-firstname" className={`formInput${errors.firstname ? " formInput--error" : ""}`} type="text" placeholder="First name" value={form.firstname} onChange={e => set("firstname", e.target.value)} autoFocus />
                              ...
                              <div className="hsf__col">
                                ...
                                <input id="hsf-lastname" className={`formInput${errors.lastname ? " formInput--error" : ""}`} type="text" placeholder="Last name" value={form.lastname} onChange={e => set("lastname", e.target.value)} />
                            ...
                            <div className="hsf__col">
                              ...
                              <input id="hsf-phone" className={`formInput${errors.phone ? " formInput--error" : ""}`} type="tel" inputMode="numeric" pattern="[0-9]*" placeholder="Phone number" value={form.phone} onChange={e => set("phone", e.target.value.replace(/[^\d-]/g, ""))} />
                        ...
                        {step === 3 && (
                          <div className="hsf__fields">
                            <div className="hsf__col">
                              ...
                              <input id="hsf-company" className={`formInput${errors.company ? " formInput--error" : ""}`} type="text" placeholder="Company name" value={form.company} onChange={e => set("company", e.target.value)} autoFocus />
                              {errors.company && <span className="fieldError">{errors.company}</span>}
                            </div>
                            <div className="hsf__col">
                              <label className="field-label" htmlFor="hsf-units">Units Managed</label>
                              <select id="hsf-units" className={`formSelect${errors.units_managed ? " formSelect--error" : ""}`} value={form.units_managed} onChange={e => set("units_managed", e.target.value)}>
                            ...
                            <div className="hsf__col">
                              <label className="field-label" htmlFor="hsf-pms">PMS Compatibility</label>
                              <select id="hsf-pms" className={`formSelect${errors.pms_compatability ? " formSelect--error" : ""}`} value={form.pms_compatability} onChange={e => set("pms_compatability", e.target.value)}>
                                <option value="">Please select</option>
                            ...
                            <div className="hsf__col">
                              <label className="field-label" htmlFor="hsf-ai">In which areas of your operations are you looking to implement AI?</label>
                              <textarea id="hsf-ai" className={`formTextarea${errors.in_which_areas_of_your_operations_are_you_looking_to_implement_ai_ ? " formTextarea--error" : ""}`} placeholder="Tell us about your goals..." rows={4} value={form.in_which_areas_of_your_operations_are_you_looking_to_implement_ai_} onChange={e => set("in_which_areas_of_your_operations_are_you_looking_to_implement_ai_", e.target.value)} />
                              {errors.in_which_areas_of_your_operations_are_you_looking_to_implement_ai_ && <span className="fieldError">{errors.in_which_areas_of_your_operations_are_you_looking_to_implement_ai_}</span>}
                            </div>
                          </div>
                        )}
                      </motion.div>
                  ...
                  <div className="hsf__nav">
                    {step < TOTAL_STEPS ? (
                      <button className="defaultButton" type="button" onClick={() => {
                        pushEvent(eventStepTwo); onEmailSubmit?.();
                        if (validateStep()) void updateContact(form.email, { firstname: form.firstname, lastname: form.lastname, phone: form.phone });
                        next();
                      }}>Continue</button>
                    ) : (
                      <button className="defaultButton" type="button" onClick={() => {
                        ...
                        if (enableWebflowEvent && (window as any).wf) {
                          (window as any).wf.ready(() => (window as any).wf.sendEvent("housing-hs-form-submit-optimize"));
                        }
                        void submit();
                      }} disabled={submitting}>
