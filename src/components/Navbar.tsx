...
interface NavbarProps {
   ...
   * solid white once past `#heroSection`. The bar is `position: fixed` so it
   * overlays the page/hero rather than taking layout space.
   * - "light" = dark text while transparent
   * - "dark" = #fafafb text / white logo / white chevrons while transparent
   * Case-insensitive ("Dark" from Webflow Variant still works).
   */
  ...
  /**
   * When true, offsets the fixed bar by 3.2rem so it sits below a sitewide
   * CTA / announcement banner. Desktop only — cleared at the mobile breakpoint
...
const parseNavItems = (navItems?: NavItem[] | string): NavItem[] => {
  ...
  if (typeof navItems === "string" && navItems.trim()) {
    try {
      const parsed = JSON.parse(navItems);
...
const Logo = () => (
  ...
  >
    ...
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M51.7568 12.6654L52.4873 10.6014H56.2349L56.9654 12.6654H58.4222L55.2563 4.17451H53.4659L50.3 12.6654H51.7568ZM60.7828 12.6654V4.17451H59.3883V12.6654H60.7828ZM54.3611 4.7442L52.6566 9.55631L52.6566 9.55631L54.3611 4.74419L54.3611 4.7442ZM52.9212 9.36929L54.3611 5.30433L55.801 9.36929H52.9212ZM60.5957 4.36154H60.5957V12.4784L60.5957 12.4784V4.36154Z"
      fill="currentColor"
    />
  </svg>
);
...
const RightPanelView = ({ panel }: { panel: RightPanel }) => {
  ...
  // panel.kind === "cta"
  // Full card art includes the copy — render as a linked image when present.
  if (panel.cta.imageUrl) {
    return (
      <a
        href={panel.cta.ctaHref}
        className="navbar__cta-card navbar__cta-card--image"
        aria-label={panel.cta.title}
      >
        <img
...
/** Fade-out duration before swapping mega-menu content between top-level links. */
const MENU_SWAP_MS = 180;
...
const MegaMenu = ({
  ...
  // Soft crossfade + height morph when hopping between top-level menu links.
  useEffect(() => {
    if (menuKey === menuKeyRef.current) {
      setDisplayMenu(menu);
      return;
    }
    menuKeyRef.current = menuKey;

    // First open / closing — swap immediately, no content fade.
    if (!hasOpenedRef.current || isClosing) {
      setDisplayMenu(menu);
      setContentVisible(true);
      return;
    }

    // Lock the painted height so the browser has a from-value to interpolate.
    const shell = shellRef.current;
    if (shell) setHeight(shell.offsetHeight);
    ...
    const t = window.setTimeout(() => {
      setDisplayMenu(menu);
      // Wait until the new content is painted, then animate height → fade in.
  ...
  useLayoutEffect(() => {
    ...
    // Swap orchestrator owns height while hopping between menus.
    if (swappingRef.current) return;

    const next = el.scrollHeight;

    // First open: start at 0 then expand so height actually animates.
  ...
  const megaClassName = `navbar__mega${
    isClosing ? " navbar__mega--closing" : " navbar__mega--open"
  }`;
    ...
    >
      <div
        ...
        className={`navbar__mega-body${
          contentVisible ? "" : " navbar__mega-body--swap"
        }`}
      >
    ...
    <div className="navbar__mobile-posts-list">
      {panel.cards.map((card, index) => (
        <a key={index} href={getLinkHref(card)} className="navbar__mobile-post">
          ...
          <div className="navbar__mobile-post-copy">
            <div className="navbar__mobile-post-category">{card.category}</div>
            <div className="navbar__mobile-post-title">{card.title}</div>
          </div>
        </a>
      ))}
    </div>
...
const MobileCtaPanel = ({
  panel,
}: {
  panel: Extract<RightPanel, { kind: "cta" }>;
}) => {
  const imageSrc = panel.cta.mobileImageUrl || panel.cta.imageUrl;

  if (imageSrc) {
    return (
      <a
        href={panel.cta.ctaHref}
        className="navbar__mobile-cta-card navbar__mobile-cta-card--image"
        aria-label={panel.cta.title}
      >
        <img
          src={imageSrc}
  ...
  return (
    <div className="navbar__mobile-cta-card">
      <div className="navbar__mobile-cta-card-content">
        <h3 className="navbar__mobile-cta-card-title">{panel.cta.title}</h3>
        <p className="navbar__mobile-cta-card-desc">{panel.cta.description}</p>
        <a href={panel.cta.ctaHref} className="navbar__mobile-cta-card-link">
          {panel.cta.ctaText}
          <svg
            className="navbar__mobile-cta-card-arrow"
            xmlns="http://www.w3.org/2000/svg"
            width="18"
            height="18"
            viewBox="0 0 18 18"
      ...
      <div className="navbar__mobile-cta-card-decor" aria-hidden="true" />
    </div>
  );
};
...
export const Navbar = ({
  ...
  const resolvedTheme = theme.toLowerCase() === "dark" ? "dark" : "light";
  ...
  // If no hero is found, stay transparent — falling back to solid would hide
  // the dark theme (white text only applies while `.navbar--transparent`).
  useEffect(() => {
  ...
  // ease back to transparent over the same duration as the menu.
  const isSolid =
    scrolledPastHero ||
    isMobileMenuOpen ||
    (Boolean(hoveredItem) && !isMenuClosing);

  // White text/logo only while transparent over the hero. Once solid, both
  // themes share the white bar + dark text (dark's only leftover difference
  // is the purple Log In link — see `.navbar--dark.navbar--solid`).
  // Logo/link colors are driven by CSS so they can ease with the menu close.

  const navClassName = [
    "navbar",
    resolvedTheme === "dark" ? "navbar--dark" : "",
    isSolid ? "navbar--solid" : "navbar--transparent",
    isMenuClosing ? "navbar--menu-closing" : "",
    ctaBannerOnTop ? "navbar--cta-banner" : "",
  ]
    .filter(Boolean)
    .join(" ");
          ...
          {activeMobileMenu ? (
            ...
            <>
              <div className="navbar__mobile-nav navbar__mobile-nav--submenu">
                {collectMobileSections(activeMobileMenu).map(
                ...
                {activeMobileMenu.variant === "split" &&
                  activeMobileMenu.right.kind === "cta" && (
                    <MobileCtaPanel panel={activeMobileMenu.right} />
                  )}
              </div>
